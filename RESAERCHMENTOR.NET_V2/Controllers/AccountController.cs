...
namespace RESAERCHMENTOR.NET_V2.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            int Ccheck = GetLoginUser(model.Email).Count();
            if (Ccheck > 0)
            {
        ...
        protected void InsertUserProfile(string email)
        {
            ...
            using (SqlConnection conAm = new SqlConnection(ConnectionState()))
            {
                ...
                try
                {
                    ...
                    var sendMail = dbconnect.ConfirmationMail(userName, userName, Message);
                    if (!sendMail)
                    {
                        Response.Write("Email server was unable to send mail, please try again letter.");
                    }
                    #endregion
                }
                catch (Exception ee)
